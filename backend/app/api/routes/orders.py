...
async def despatch_order(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Unable to generate despatch advice.",
        ) from exc
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail="Unable to persist despatch advice.",
        ) from exc
...
def get_despatch_xml(
    ...
    try:
        result = getXml("dispatch_xml", order_id)
    except Exception as exc:
        logger.exception("Failed to fetch despatch XML")
        raise HTTPException(
            status_code=500,
    ...
    if not xml:
        ...
        raise HTTPException(
            status_code=500,
            detail="Despatch XML missing or corrupted.",
        )
